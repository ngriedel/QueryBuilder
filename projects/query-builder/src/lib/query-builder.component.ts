...
export class QueryBuilderComponent implements OnInit, OnChanges, ControlValueAccessor, Validator {
  ...
  public defaultOperatorMap: { [key: string]: OperatorOption[] } = {
    string: [
      { op: '=', hint: 'equals value' },
      { op: '!=', hint: 'not equals value' },
      { op: 'contains', hint: 'contains value' },
      { op: 'like', hint: 'matches pattern' }
    ],
    number: [
      { op: '=', hint: 'equals value' },
      { op: '!=', hint: 'not equals value' },
      { op: '>', hint: 'greater than value' },
      { op: '>=', hint: 'greater than or equals value' },
      { op: '<', hint: 'less than value' },
      { op: '<=', hint: 'less than or equals value' }
    ],
    time: [
      { op: '=', hint: 'equals time' },
      { op: '!=', hint: 'not equals time' },
      { op: '>', hint: 'after time' },
      { op: '>=', hint: 'at or after time' },
      { op: '<', hint: 'before time' },
      { op: '<=', hint: 'at or before time' }
    ],
    date: [
      { op: '=', hint: 'equals date' },
      { op: '!=', hint: 'not equals date' },
      { op: '>', hint: 'after date' },
      { op: '>=', hint: 'at or after date' },
      { op: '<', hint: 'before date' },
      { op: '<=', hint: 'at or before date' }
    ],
    category: [
      { op: '=', hint: 'equals value' },
      { op: '!=', hint: 'not equals value' },
      { op: 'in', hint: 'in list' },
      { op: 'not in', hint: 'not in list' }
    ],
    boolean: [
      { op: '=', hint: 'equals value' }
    ]
  ...
  @Input() defaultValue: RuleSet = { condition: 'and', rules: [] };

  // For ControlValueAccessor interface
  public onChangeCallback: () => void;
  public onTouchedCallback: () => any;
  public onDisabledChangeCallback: (isDisabled: boolean) => void;
  ...
  private operatorsCache: { [key: string]: (string | OperatorOption)[] };
  private inputContextCache = new Map<RuleSet, InputContext>();
  private operatorContextCache = new Map<RuleSet, OperatorContext>();
  private fieldContextCache = new Map<RuleSet, FieldContext>();
  private entityContextCache = new Map<RuleSet, EntityContext>();
  private removeButtonContextCache = new Map<RuleSet, RemoveButtonContext>();
  ...
  ngOnInit() {
    // Apply default value if data is empty
    if (!this.data || !this.data.rules || this.data.rules.length === 0) {
      this.data = { ...this.defaultValue };
    }
  }

  // This is already implemented but included for reference
  registerOnChange(fn: any): void {
    this.onChangeCallback = () => fn(this.data);
  }

  // New method to register disabled state change callback
  registerOnDisabledChange(fn: (isDisabled: boolean) => void): void {
    this.onDisabledChangeCallback = fn;
  }

  setDisabledState(isDisabled: boolean): void {
    this.disabled = isDisabled;
    // Notify subscribers about disabled state change
    if (this.onDisabledChangeCallback) {
      this.onDisabledChangeCallback(isDisabled);
    }
...
//   }
  ...
  findTemplateForRule(rule: RuleSet): TemplateRef<any> {
  ...
  getOperators(field: string): (string | OperatorOption)[] {
    ...
    if (fieldObject && fieldObject.operators) {
      ...
      if (fieldObject.nullable) {
        operators = operators.concat([
          { op: 'is null', hint: 'has no value' },
          { op: 'is not null', hint: 'has a value' }
        ]);
      }
    } else {
  ...
  getOperatorDisplay(operator: string | OperatorOption): string {
    return typeof operator === 'string' ? operator : operator.op;
  }
  ...
  getDefaultOperator(field: Field): string {
    if (field && field.defaultOperator !== undefined) {
      ...
      if (operators && operators.length) {
        ...
        // Handle both string and OperatorOption formats
  ...
  changeOperator(rule: RuleSet): void {
    if (this.disabled) {
      return;
    }
  ...
  changeField(fieldValue: string, rule: RuleSet): void {
    if (this.disabled) {
      return;
    }
  ...
  changeEntity(entityValue: string, rule: RuleSet, index: number, data: RuleSet): void {
    if (this.disabled) {
      return;
    }
    let i = index;
  ...
  getRemoveButtonContext(rule: RuleSet): RemoveButtonContext {
  ...
  getFieldContext(rule: RuleSet): FieldContext {
  ...
  getEntityContext(rule: RuleSet): EntityContext {
  ...
  getOperatorContext(rule: RuleSet): OperatorContext {
  ...
  getInputContext(rule: RuleSet): InputContext {
  ...
  private validateRulesInRuleset(ruleset: RuleSet, errorStore: any[]) {
    if (ruleset && ruleset.rules && ruleset.rules.length > 0) {
      ruleset.rules.forEach((item) => {
        ...
        } else if ((item as RuleSet).field) {
          const field = this.config.fields[(item as RuleSet).field];
          if (field && field.validator && field.validator.apply) {
            const error = field.validator(item as RuleSet, ruleset);
            if (error != null) {
